...
class Auth extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model(array('Proposal_model', 'PD_model', 'Auth_model'));

    $this->data['btn_submit'] = 'Save';
    $this->data['btn_reset']  = 'Reset';
    $this->data['btn_add']    = 'Tambah Data Baru';
    $this->data['btn_info']    = 'Akun Baru';
    $this->data['add_action'] = base_url('auth/create');
    $this->data['add_admin'] = base_url('auth/create_admin');
    $this->data['user_new'] = base_url('auth/user_new');
  }
  ...
  function index()
  {
    ...
    $this->data['page_title'] = $this->data['module'] . ' Terverifikasi';
    $this->data['get_all'] = $this->Auth_model->get_all();
    $this->data['get_by_createat'] = $this->Auth_model->get_by_createat();
  ...
  function user_new()
  {
    ...
    $this->data['page_title'] = $this->data['module'] . ' Terdaftar Hari Ini';

    $this->data['get_by_createat'] = $this->Auth_model->get_by_createat();
    $this->load->view('back/auth/user_list_new', $this->data);
  }
      ...
      if ($_FILES['photo']['error'] <> 4) {
        ...
        if (!$this->upload->do_upload('photo')) {
          ...
          $data = array(
            ...
            'photo'             => 'ktp_' . $this->upload->data('file_name'),
            'photo_thumb'       => $nmfile . '_thumb' . $this->upload->data('file_ext'),
          );
  ...
  function update_profile_action()
  {
    ...
    if ($this->form_validation->run() === FALSE) {
      ...
      if ($_FILES['photo']['photonpwp']['error'] <> 4) {
        $nmfile = strtolower(url_title($this->input->post('username'))) . date('YmdHis');
        ...
        if (!$this->upload->do_upload('photo') && !$this->upload->do_upload('photonpwp')) {
          $error = array('error' => $this->upload->display_errors());
          $this->session->set_flashdata('message', '<div class="alert alert-danger">' . $error['error'] . '</div>');
  ...
  function register()
  {
    ...
    $this->form_validation->set_rules('name', 'Full Name', 'trim|required');
    $this->form_validation->set_rules('nohp', 'Phone No', 'trim|is_numeric');
    $this->form_validation->set_rules('username', 'Username', 'trim|is_unique[users.username]|required');
    $this->form_validation->set_rules('email', 'Email', 'valid_email|is_unique[users.email]|required');
    $this->form_validation->set_rules('password', 'Password', 'trim|min_length[8]|required');
    $this->form_validation->set_rules('password_confirm', 'Password Confirmation', 'trim|matches[password]|required');
    ...
    if ($this->form_validation->run() === FALSE) {
      ...
      if ($_FILES['photo']['error'] <> 4) {
        ...
        if (!$this->upload->do_upload('photo')) {
          ...
          $this->session->set_flashdata('message', '<div class="alert alert-success">Tunggu Konfirmasi Admin</div>');
          redirect('auth/login');
        }
      } else {
        $nmfile = strtolower(url_title($this->input->post('username'))) . date('YmdHis');
  ...
  function register_form()
  {
    ...
    $this->data['page_title'] = "Daftar Akun";
    $this->data['register']     = 'auth/register';
    ...
    $this->load->view('back/auth/register', $this->data);
  }
