...
$(document).ready(function($) {
    ...
    if( customizerEnabled == 1 ){
        $.getScript( "frontEnd/misc/customizer.js", function( data, textStatus, jqxhr ) {
...
// Render hero search form ---------------------------------------------------------------------------------------------
    ...
    $("select").on("rendered.bs.select", function () {
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'frontEnd/css/bootstrap-select.min.css') );
        if( !viewport.is('xs') ){
    ...
    $(".form-email .btn[type='submit']").on("click", function(){
        ...
        form.validate({
            submitHandler: function() {
                $.post("frontEnd/external/email.php", form.serialize(),  function(response) {
        ...
        setTimeout(function(){
            if( _this.find(".map").length ){
                if( _this.find(".modal-dialog").attr("data-address") ){
                    simpleMap(Url, 0, 0, "map-modal", _this.find(".modal-dialog").attr("data-marker-drag"), _this.find(".modal-dialog").attr("data-address") );
                }
                else {
                    simpleMap(Url, _this.find(".modal-dialog").attr("data-latitude"), _this.find(".modal-dialog").attr("data-longitude"), "map-modal", _this.find(".modal-dialog").attr("data-marker-drag") );
                }
            }
            initializeOwl();
            initializeFitVids();
...
function socialShare(){
    ...
    if ( socialButtonsEnabled == 1 ){
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'frontEnd/css/jssocials.css') );
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'frontEnd/css/jssocials-theme-minima.css') );
        $.getScript( "frontEnd/js/jssocials.min.js", function( data, textStatus, jqxhr ) {
...
function initializeReadMore(){
    ...
    $.ajax({
        type: "GET",
        url: "frontEnd/js/readmore.min.js",
